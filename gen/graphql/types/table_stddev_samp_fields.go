...
func buildTableStddevSampFields(ctx *context.GqlBuildContext) *ast.Definition {
	def := &ast.Definition{}
	def.Kind = ast.Object
	def.Description = fmt.Sprintf(`aggregate stddev_samp on columns of table "%s"`, ctx.TableName)
	def.Name = TableStddevSampFieldsName(ctx)
	// build fields
...
func TableStddevSampFieldsName(ctx *context.GqlBuildContext) string {
	return strcase.ToCamel(ctx.TableName + "_stddev_samp_fields")
}

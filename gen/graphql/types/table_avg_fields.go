...
func buildTableAvgFields(ctx *context.GqlBuildContext) *ast.Definition {
	def := &ast.Definition{}
	def.Kind = ast.Object
	def.Description = fmt.Sprintf(`aggregate avg on columns of table "%s"`, ctx.TableName)
	def.Name = TableAvgFieldsName(ctx)
	// build fields
...
func TableAvgFieldsName(ctx *context.GqlBuildContext) string {
	return strcase.ToCamel(ctx.TableName + "_avg_fields")
}

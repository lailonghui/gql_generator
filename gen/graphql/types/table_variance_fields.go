...
func buildTableVarianceFields(ctx *context.GqlBuildContext) *ast.Definition {
	def := &ast.Definition{}
	def.Kind = ast.Object
	def.Description = fmt.Sprintf(`aggregate variance on columns of table "%s"`, ctx.TableName)
	def.Name = TableVarianceFieldsName(ctx)
	// build fields
...
func TableVarianceFieldsName(ctx *context.GqlBuildContext) string {
	return strcase.ToCamel(ctx.TableName + "_variance_fields")
}

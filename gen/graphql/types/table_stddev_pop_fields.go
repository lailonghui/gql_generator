...
func buildTableStddevPopFields(ctx *context.GqlBuildContext) *ast.Definition {
	def := &ast.Definition{}
	def.Kind = ast.Object
	def.Description = fmt.Sprintf(`aggregate stddev_pop on columns of table "%s"`, ctx.TableName)
	def.Name = TableStddevPopFieldsName(ctx)
	// build fields
...
func TableStddevPopFieldsName(ctx *context.GqlBuildContext) string {
	return strcase.ToCamel(ctx.TableName + "_stddev_pop_fields")
}

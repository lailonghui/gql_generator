...
func buildTableVarSampFields(ctx *context.GqlBuildContext) *ast.Definition {
	def := &ast.Definition{}
	def.Kind = ast.Object
	def.Description = fmt.Sprintf(`aggregate var_samp on columns of table "%s"`, ctx.TableName)
	def.Name = TableVarSampFieldsName(ctx)
	// build fields
...
func TableVarSampFieldsName(ctx *context.GqlBuildContext) string {
	return strcase.ToCamel(ctx.TableName + "_var_samp_fields")
}

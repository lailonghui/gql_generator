...
func buildTableVarPopFields(ctx *context.GqlBuildContext) *ast.Definition {
	def := &ast.Definition{}
	def.Kind = ast.Object
	def.Description = fmt.Sprintf(`aggregate var_pop on columns of table "%s"`, ctx.TableName)
	def.Name = TableVarPopFieldsName(ctx)
	// build fields
...
func TableVarPopFieldsName(ctx *context.GqlBuildContext) string {
	return strcase.ToCamel(ctx.TableName + "_var_pop_fields")
}

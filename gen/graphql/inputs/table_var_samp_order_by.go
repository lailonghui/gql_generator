...
func buildTableVarSampOrderBy(ctx *context.GqlBuildContext) *ast.Definition {
	def := &ast.Definition{}
	def.Kind = ast.InputObject
	def.Description = fmt.Sprintf(`order by stddev_pop() on columns of table "%s"`, ctx.TableName)
	def.Name = TableVarSampOrderByName(ctx)
	// build fields
...
func TableVarSampOrderByName(ctx *context.GqlBuildContext) string {
	return strcase.ToCamel(ctx.TableName + "_var_samp_order_by")
}

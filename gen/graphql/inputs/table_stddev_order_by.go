...
func buildTableStddevOrderBy(ctx *context.GqlBuildContext) *ast.Definition {
	def := &ast.Definition{}
	def.Kind = ast.InputObject
	def.Description = fmt.Sprintf(`order by stddev() on columns of table "%s"`, ctx.TableName)
	def.Name = TableStddevOrderByName(ctx)
	// build fields
...
func TableStddevOrderByName(ctx *context.GqlBuildContext) string {
	return strcase.ToCamel(ctx.TableName + "_stddev_order_by")
}

...
func buildTableStddevSampOrderBy(ctx *context.GqlBuildContext) *ast.Definition {
	def := &ast.Definition{}
	def.Kind = ast.InputObject
	def.Description = fmt.Sprintf(`order by stddev_samp() on columns of table "%s"`, ctx.TableName)
	def.Name = TableStddevSampOrderByName(ctx)
	// build fields
...
func TableStddevSampOrderByName(ctx *context.GqlBuildContext) string {
	return strcase.ToCamel(ctx.TableName + "_stddev_samp_order_by")
}

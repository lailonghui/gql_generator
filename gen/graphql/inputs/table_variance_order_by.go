...
func buildTableVarianceOrderBy(ctx *context.GqlBuildContext) *ast.Definition {
	def := &ast.Definition{}
	def.Kind = ast.InputObject
	def.Description = fmt.Sprintf(`order by stddev_pop() on columns of table "%s"`, ctx.TableName)
	def.Name = TableVarianceOrderByName(ctx)
	// build fields
...
func TableVarianceOrderByName(ctx *context.GqlBuildContext) string {
	return strcase.ToCamel(ctx.TableName + "_variance_order_by")
}
